...
Object.assign(Plugin.prototype, {
  ...
  close() {
    if (_$popup) {
      if (!H.haveMouse()) {
        bootstrap.Modal.getInstance(_$popup[0]).hide();
      } else {
  ...
  _open(d, refresh) {
    ...
    if (refresh) {
      _$popup[0].querySelector('.content').innerHTML = content;

      // Resize only if popover
      if (_$popup[0].classList.contains('popover')) {
